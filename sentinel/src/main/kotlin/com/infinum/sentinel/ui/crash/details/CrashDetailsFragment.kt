...
internal class CrashDetailsFragment :
        ...
        when (state) {
            is CrashDetailsState.Data -> {
                with(binding) {
                    ...
                    stackTraceView.text =
                        "${state.value.data.exception?.name}: ${state.value.data.exception?.message}"
                            .plus(
                                state.value.data.exception
                                    ?.stackTrace
                                    ?.joinToString { "\n\t\t\t at $it" },
                    ...
                    if (state.value.data.exception
                        ...
                        state.value.data.threadState?.forEach { process ->
                            threadStatesContainer.addView(
                                SentinelViewItemThreadStateBinding
                                    .inflate(layoutInflater, threadStatesContainer, false)
                                    .apply {
                                        stackTraceView.text =
    ...
    override fun onEvent(event: CrashDetailsEvent) =
        when (event) {
            is CrashDetailsEvent.Removed -> {
                requireActivity().finish()
            }

            is CrashDetailsEvent.Formatted -> {
                ShareCompat
