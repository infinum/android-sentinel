...
internal inline fun <reified VM : ViewModel> ComponentActivity.viewModels(): Lazy<VM> =
    viewModels { InjectedViewModelFactory }

internal inline fun <reified VM : ViewModel> ComponentActivity.viewModels(
...
internal inline fun <reified VM : ViewModel> Fragment.viewModels(): Lazy<VM> =
    viewModels { InjectedViewModelFactory }
...
internal class InjectedSavedStateViewModelFactory(
    ...
    override fun <T : ViewModel> create(key: String, modelClass: Class<T>, handle: SavedStateHandle): T {
        return if (modelClass.constructors.any { it.parameterTypes.contains(SavedStateHandle::class.java) }) {
            (LibraryComponents.viewModels().viewModelMap[modelClass] as T).let { clazz ->
                clazz.javaClass.constructors
                    ...
                    ?.parameterTypes?.find { it == SavedStateHandle::class.java }
                    ?.let { param ->
